...
react는 싱글 페이지라서 uri 요청이 다른 페이지에서 이루어지면 index.html로 리다이렉션 해주어야 한다.
try_files $uri $uri/ /index.html;
...
jenkins tools 설정(local 환경과 동일하게 버전 맞춰야 함!)
...
jenkins push event 설정(master 브랜치에 푸시하면 자동으로 배포 및 빌드 진행)
<jenkins page>
item 생성
        ...
        stage('BackEnd Deploy to EC2'){
            steps {
                ...
                sshagent(credentials: ['aws-key']) {
                    ...
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@i10E107.p.ssafy.io "sudo docker run --name spring -p 8080:8080 -d ssj0187/gollajyu:1.0"'
                }
            }
        }
    }
